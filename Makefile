...
ANDROID_CURRENT_VERSION = 6.0.1
ANDROID_IMAGES = 6.0.1
...
android: 
	...
	sed -i "s/###-->ZZZ_CURRENT_VERSION<--###/$(ANDROID_CURRENT_VERSION)/g" $(CURRENT_DIR)/$@/$$tf_ver/README.md ; \
	sed -i "s/###-->ZZZ_VERSION<--###/$$tf_ver/g" $(CURRENT_DIR)/$@/$$tf_ver/README.md ; \
	sed -i "s/###-->ZZZ_ANDROID_VERSION<--###/$$tf_ver/g" $(CURRENT_DIR)/$@/$$tf_ver/README.md ; \
	done
...
android_rm:
	@for tf_ver in $(ANDROID_IMAGES); \
	do \
	echo "Removing '$$tf_ver android' image..." ; \
	echo " " ; \
	if $(DOCKER_BIN) images $(NAME)/odroidc2-android | awk '{ print $$2 }' | grep -q -F $$tf_ver; then $(DOCKER_BIN) rmi -f $(NAME)/odroidc2-android:$$tf_ver; fi ; \
	done
...
.PHONY: misc_test
misc_test: uboot_test linux_test android_test
	@echo " "
	@echo " "
...
.PHONY: misc_rm
misc_rm: uboot_rm linux_rm android_rm
...
tag_latest:
	...
	$(DOCKER_BIN) tag $(NAME)/odroidc2-android:$(ANDROID_CURRENT_VERSION) $(NAME)/odroidc2-android:latest
...
release: tag_latest
	...
	@if ! $(DOCKER_BIN) images $(NAME)/odroidc2-android | awk '{ print $$2 }' | grep -q -F $(ANDROID_CURRENT_VERSION); then echo "$(NAME)/odroidc2-android android version $(ANDROID_CURRENT_VERSION) is not yet built. Please run 'make build'"; false; fi
	$(DOCKER_BIN) push $(NAME)/odroidc2-uboot
	$(DOCKER_BIN) push $(NAME)/odroidc2-linux
	$(DOCKER_BIN) push $(NAME)/odroidc2-android
